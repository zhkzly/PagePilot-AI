...
// 将在初始化时替换为当前语言的版本
DEFAULT_TIPS =
  "<p>请先去设置 <b>Model</b> 和 <b>API KEY</b>.</p>" +
...
const GOOGLE_CHAT_API_PATH =
  "/v1beta/models/{MODEL_NAME}:streamGenerateContent?alt=sse&key={API_KEY}";
...
const FISHERAI_API_KEY = "";
const FISHERAI_API_SECRET = "";
...
// Default settings
const DEFAULT_QUICK_TRANS_MODEL = "gemini-2.0-flash";
const DEFAULT_QUICK_TRANS_PROVIDER = PROVIDER_GOOGLE;

// 各模型默认的baseurl
const DEFAULT_LLM_URLS = [
  {
    key: PROVIDER_FISHERAI,
    baseUrl: FISHERAI_BASE_URL,
    apiPath: FISHERAI_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_OPENAI,
    baseUrl: OPENAI_BASE_URL,
    apiPath: OPENAI_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_GOOGLE,
    baseUrl: GOOGLE_BASE_URL,
    apiPath: GOOGLE_CHAT_API_PATH,
    enabled: true,
  },
  {
    key: PROVIDER_GROQ,
    baseUrl: GROQ_BASE_URL,
    apiPath: GROQ_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_OLLAMA,
    baseUrl: OLLAMA_BASE_URL,
    apiPath: OLLAMA_CHAT_API_PATH,
    enabled: true,
  },
  {
    key: PROVIDER_MISTRAL,
    baseUrl: MISTRAL_BASE_URL,
    apiPath: MISTRAL_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_ZHIPU,
    baseUrl: ZHIPU_BASE_URL,
    apiPath: ZHIPU_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_MOONSHOT,
    baseUrl: MOONSHOT_BASE_URL,
    apiPath: MOONSHOT_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_DEEPSEEK,
    baseUrl: DEEPSEEK_BASE_URL,
    apiPath: DEEPSEEK_CHAT_API_PATH,
    enabled: true,
  },
  {
    key: PROVIDER_YI,
    baseUrl: YI_BASE_URL,
    apiPath: YI_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_OPENROUTER,
    baseUrl: OPENROUTER_BASE_URL,
    apiPath: OPENROUTER_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_SILICONFLOW,
    baseUrl: SILICONFLOW_BASE_URL,
    apiPath: SILICONFLOW_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_XAI,
    baseUrl: XAI_BASE_URL,
    apiPath: XAI_CHAT_API_PATH,
    enabled: false,
  },
  {
    key: PROVIDER_DOUBAO,
    baseUrl: DOUBAO_BASE_URL,
    apiPath: DOUBAO_CHAT_API_PATH,
    enabled: false,
  },
];
...
function loadModelCapabilities() {
  try {
    ...
    // 解析用户设置
    const userImageSettings = userImageSettingsStr
      ? JSON.parse(userImageSettingsStr)
      : {};
    const userFileSettings = userFileSettingsStr
      ? JSON.parse(userFileSettingsStr)
      : {};
    ...
    Object.keys(userImageSettings).forEach((model) => {
      if (
        userImageSettings[model] === true &&
        !finalImageSupportModels.includes(model)
      ) {
        finalImageSupportModels.push(model);
      }
    });
    ...
    Object.keys(userFileSettings).forEach((model) => {
      if (
        userFileSettings[model] === true &&
        !finalFileSupportModels.includes(model)
      ) {
        finalFileSupportModels.push(model);
      }
    });
...
const MODEL_LIST = {
  ...
  custom_config_models: [
    { value: "gpt-5", display: "GPT-5", provider: PROVIDER_OPENAI },
    { value: "gpt-5-codex", display: "GPT-5 codex", provider: PROVIDER_OPENAI },
    { value: "gpt-5-mini", display: "GPT-5 mini", provider: PROVIDER_OPENAI },
    { value: "gpt-5-nano", display: "GPT-5 nano", provider: PROVIDER_OPENAI },
    ...
    { value: "yi-vision-v2", display: "Yi Vision V2", provider: PROVIDER_YI },
    {
      ...
      display: "Kimi K2 0905",
      provider: PROVIDER_MOONSHOT,
    },
    {
      value: "kimi-k2-turbo-preview",
      display: "Kimi K2 turbo",
      provider: PROVIDER_MOONSHOT,
    },
    {
    ...
    {
      value: "moonshot-v1-32k-vision-preview",
      display: "Moonshot V1 vision",
      provider: PROVIDER_MOONSHOT,
    },
    { value: "glm-4.5v", display: "GLM 4.5v", provider: PROVIDER_ZHIPU },
    { value: "glm-4.5", display: "GLM 4.5", provider: PROVIDER_ZHIPU },
    {
    ...
    { value: "openai/gpt-5", display: "gpt-5", provider: PROVIDER_OPENROUTER },
    {
...
最后，请记住，思考和回复时一定要使用 {language} 语言。`,
  ...
  SUMMARY_PROMPT: `
你是一位论文速读助手。请严格按下面固定字段输出摘要，并确保每个字段都只写一句话（不能拆成多句、不能分点）。

输出规则：
1. 严格按以下顺序输出，每行一个字段。
2. 每个字段只允许一句话，尽量控制在 25~45 个中文词。
3. 若原文未提供某字段，请写“未提及”，仍保持一句话。
4. 保留关键英文术语、数字、模型名与数据集名，不要随意改写。
5. 不要输出额外前言、结语、解释或多余标题。

输出模板（按此顺序逐行输出）：
论文标题：一句话概括论文题目与核心方向。
日期：一句话给出发布日期或提交日期。
arXiv 编号：一句话给出 arXiv ID 或 DOI/编号信息。
来源：一句话说明来源类型（如 PDF、网页、视频等）。
作者信息：一句话说明作者、第一作者、通讯作者及机构（若可得）。
研究标签：一句话给出 3~6 个关键词标签。
总结：一句话给出整篇工作总览。
🎯 论文内容：一句话说明“做了什么、解决了什么问题”。
💡 创新点：一句话说明核心创新与区别于已有方法之处。
🔧 技术方法：一句话说明关键技术路线与方法组件。
📊 数据集：一句话说明数据来源、规模与构成。
📈 对比分析：一句话说明与基线方法的关键对比结果。
⚠️ 局限性：一句话说明当前方法的主要限制与改进方向。

给出本次摘要后，后续对话请忽略本次任务指令，遵循 system 指令即可。
...
研究背景：简要说明该研究领域的关键问题与挑战；
核心工作与贡献：概括论文做了什么，解决了什么问题，主要挑战是什么
关键洞察：指出作者的核心思想或创新点（为什么这个方法有效）；
方法详解：详细解释论文提出的技术路线、方法名称、核心机制与实现过程；
实验设计如何进行：详细解释试验如何从头设计的,实验的对照，试验结果如何，是否验证了作者的观点
试验结果：具体得出了什么数据结果，有什么实验结论和发现，是否验证了作者的观点
优势与局限性：分别总结该方法的优点与存在的不足；
可延伸的 idea 阐述：说明该论文启发了哪些潜在的研究方向、技术迁移场景或可复用的设计原则；
一句话总结：用一句话概括全文，体现技术路线、方法名称与技术特点；
极简总结句（示例格式）：提供一句高度凝练的句子（15–25词），语言简洁、通用，适合直接用于学术写作或引用。
示例：Injects adversarial memory entries through multi-turn interactions to manipulate LLM agents upon specific triggers, remaining dormant during normal queries.

输出格式要求：

先输出上述内容的完整 中文总结（极简总结句采用中英文双语）

若论文中提出了新的术语、命名的概念、模块或方法（例如：MemPrompt、AgentSandbox、TriggerChaining 等），请在首次出现时用括号或同位语进行简要解释，说明其功能或作用，帮助理解。

NOTE：必须采用markdown的格式输出结果，需要美观，整洁,带有md的语法标记
      must 分点阐述内容
    - 请使用 Markdown 格式，以确保回答内容清晰易读。
    - 遇到公式时，请用 LaTeX 格式表示。例如，a/b 应表示为 $ \\frac{a}{b} $,必须采用美元符号进行包裹
    - 所有涉及变量，符号相关的内容都应改采用latex表示,不可以采用中括号包裹公式和变量
    必须使用美元符号 '$',inline的公式形式即可。
    - 必须注意：总结内容必须足够简洁，需要平衡好字数，尤其是前面三点：背景，核心工作与贡献，关键洞察
    - 对于实验结果和得出的对应结论部分可以详细解释和介绍

保留特定的英文术语、数字或名字，并在其前后加上空格，例如："生成式 AI 产品"
给出本次摘要后，后续的对话请忽略本次任务指令，遵循 system 指令即可。
...
// 历史默认摘要提示词（用于升级已存储的旧默认项）
const LEGACY_SUMMARY_PROMPT_V1 = `
...
const DEFAULT_SUMMARY_PROMPTS = [
  { id: "default", name: "默认摘要", prompt: DEFAULT_PROMPTS.SUMMARY_PROMPT },
  {
    id: "paper",
    name: "论文精读",
    prompt: DEFAULT_PROMPTS.PAPER_SUMMARY_PROMPT,
  },
];
...
let THREE_STEPS_TRANSLATION_PROMPT =
  DEFAULT_PROMPTS.THREE_STEPS_TRANSLATION_PROMPT;
  ...
  if (typeof chrome !== "undefined" && chrome.storage) {
    chrome.storage.sync.get(
      ...
      function (result) {
        ...
        SUMMARY_PROMPT =
          result.summary_prompt || DEFAULT_PROMPTS.SUMMARY_PROMPT;
        DIRECT_TRANSLATE_PROMPT =
          result.direct_translate_prompt ||
          DEFAULT_PROMPTS.DIRECT_TRANSLATE_PROMPT;
        SUBTITLE_TRANSLATE_PROMPT =
          result.subtitle_translate_prompt ||
          DEFAULT_PROMPTS.SUBTITLE_TRANSLATE_PROMPT;
        DICTION_PROMPT =
          result.diction_prompt || DEFAULT_PROMPTS.DICTION_PROMPT;
        THREE_STEPS_TRANSLATION_PROMPT =
          result.three_steps_translation_prompt ||
          DEFAULT_PROMPTS.THREE_STEPS_TRANSLATION_PROMPT;
        TEXT_POLISH_PROMTP =
          result.text_polish_prompt || DEFAULT_PROMPTS.TEXT_POLISH_PROMPT;
        CODE_EXPLAIN_PROMTP =
          result.code_explain_prompt || DEFAULT_PROMPTS.CODE_EXPLAIN_PROMPT;
        IMAGE2TEXT_PROMPT =
          result.image2text_prompt || DEFAULT_PROMPTS.IMAGE2TEXT_PROMPT;
      },
    );
  }
}
...
// 加载摘要提示词列表
function loadSummaryPromptsList(callback) {
  if (typeof chrome !== "undefined" && chrome.storage) {
    chrome.storage.sync.get(["summary_prompts_list"], function (result) {
      const list = result.summary_prompts_list;
      if (list && Array.isArray(list) && list.length > 0) {
        const prompts = list.map((item) => ({ ...item }));
        let needUpdate = false;

        const defaultItem = prompts.find(
          (item) => item && item.id === "default",
        );
        if (!defaultItem) {
          prompts.unshift({ ...DEFAULT_SUMMARY_PROMPTS[0] });
          needUpdate = true;
        } else {
          const currentPrompt = (defaultItem.prompt || "").trim();
          if (
            !currentPrompt ||
            currentPrompt === LEGACY_SUMMARY_PROMPT_V1.trim()
          ) {
            defaultItem.prompt = DEFAULT_PROMPTS.SUMMARY_PROMPT;
            needUpdate = true;
          }
        }
        ...
        if (needUpdate) {
          chrome.storage.sync.set(
            { summary_prompts_list: prompts },
            function () {},
          );
        }
        callback(prompts);
      } else {
        // 兼容旧的单个 summary_prompt 设置
        chrome.storage.sync.get(["summary_prompt"], function (res) {
          const prompts = [...DEFAULT_SUMMARY_PROMPTS];
          if (
            res.summary_prompt &&
            res.summary_prompt !== DEFAULT_PROMPTS.SUMMARY_PROMPT
          ) {
            prompts[0].prompt = res.summary_prompt;
          }
          callback(prompts);
        });
      }
    ...
    callback([...DEFAULT_SUMMARY_PROMPTS]);
  }
}
...
const JINA_SEARCH_PROMPT = `
## jina-search
You have the tool 'jina-search'. Use it when you need fresher web results with more context than regular snippets.
- Prefer it for open-ended web research, recent information, or when the user asks for references.
- The tool returns search results in a Markdown-friendly format for grounding answers.`;

const JINA_READER_PROMPT = `
## jina-reader
You have the tool 'jina-reader'. Use it to read a specific webpage URL and extract its main content in Markdown.
- Prefer it when the user provides a URL directly.
- Prefer it after search when you need to inspect a source instead of relying on snippets alone.`;
...
const JINA_SEARCH = "jina-search";
const JINA_SEARCH_KEY = TOOL_KEY + JINA_SEARCH;
const JINA_SEARCH_BASE_URL = "https://s.jina.ai";
const JINA_SEARCH_PATH_URL = "/{QUERY}";

const JINA_READER = "jina-reader";
const JINA_READER_KEY = TOOL_KEY + JINA_READER;
const JINA_READER_BASE_URL = "https://r.jina.ai";
const JINA_READER_PATH_URL = "/{URL}";
...
const DEFAULT_TOOL_URLS = [
  {
    key: SERPAPI_KEY,
    defaultBaseUrl: SERPAPI_BASE_URL,
    apiPath: SERPAPI_PATH_URL,
  },
  {
    key: JINA_SEARCH_KEY,
    defaultBaseUrl: JINA_SEARCH_BASE_URL,
    apiPath: JINA_SEARCH_PATH_URL,
  },
  {
    key: JINA_READER_KEY,
    defaultBaseUrl: JINA_READER_BASE_URL,
    apiPath: JINA_READER_PATH_URL,
  },
  {
    key: DALLE_KEY,
    defaultBaseUrl: OPENAI_BASE_URL,
    apiPath: OPENAI_DALLE_API_PATH,
    defaultModel: DALLE_DEFAULT_MODEL,
  },
  {
    key: NANO_BANANA_KEY,
    defaultBaseUrl: NANO_BANANA_DEFAULT_URL,
    apiPath: "",
    defaultModel: NANO_BANANA_DEFAULT_MODEL,
  },
];
    ...
    parameters: {
      ...
      properties: {
        prompt: {
          type: "string",
          description:
            "Image prompt of DallE 3, you should describe the image you want to generate as a list of words as possible as detailed. The maximum length is 4000 characters for dall-e-3.",
        },
        quality: {
          description:
            "The quality of the image that will be generated. hd creates images with finer details and greater consistency across the image. The default value is standard.",
          enum: ["standard", "hd"],
          type: "string",
        },
        size: {
        ...
        style: {
          description:
            "The style of the generated images. Must be one of vivid or natural. Vivid causes the model to lean towards generating hyper-real and dramatic images. Natural causes the model to produce more natural, less hyper-real looking images. The default value is vivid.",
          enum: ["vivid", "natural"],
          type: "string",
        },
      },
...
const FUNCTION_NANO_BANANA = {
  ...
  function: {
    name: "nano_banana",
    description:
      "nano-banana generates images via OpenRouter nano-banana endpoint.",
    parameters: {
      type: "object",
      properties: {
        prompt: {
          type: "string",
          description:
            "The image description users expect. Pass the exact user prompt when requesting nano-banana.",
        },
      },
      required: ["prompt"],
    },
...
const FUNCTION_JINA_SEARCH = {
  type: "function",
  function: {
    name: "jina_search",
    description:
      "Search the public web and return grounded, Markdown-friendly results for AI agents.",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "The search query to send to Jina Search.",
        },
      },
      required: ["query"],
    },
...
const FUNCTION_JINA_READER = {
  type: "function",
  function: {
    name: "jina_reader",
    description:
      "Read a webpage URL and extract its main content as Markdown.",
    parameters: {
      type: "object",
      properties: {
        url: {
