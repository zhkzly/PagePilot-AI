...
function getToolsParamForCheck(baseUrl, tabId, apiKey, modelName = '') {
  ...
  for (const { key, defaultBaseUrl, apiPath, defaultModel } of DEFAULT_TOOL_URLS) {
    if(tabId.includes(key)) {
      ...
      } else if(tabId.includes(JINA_SEARCH_KEY)) {
        apiUrl = apiUrl.replace('{QUERY}', encodeURIComponent('latest ai news'));
      } else if(tabId.includes(JINA_READER_KEY)) {
        apiUrl = apiUrl.replace('{URL}', 'http://example.com');
...
function checkAPIAvailable(baseUrl, apiKey, tabId, resultElement, modelName = '') {
  ...
  const checkAPI = async () => {
    ...
    if(!tabId.includes(PROVIDER_GOOGLE) && apiKey) {
    ...
    if(tabId.includes(PROVIDER_OLLAMA) || tabId.includes(SERPAPI_KEY) || tabId.includes(JINA_SEARCH_KEY) || tabId.includes(JINA_READER_KEY)) {
      params = {
        method: "GET",
        headers: headers
      }
...
function setupPromptSettings() {
  ...
  // === 摘要提示词列表管理 ===
  let summaryPromptsList = [];
  const summaryPromptsListEl = document.getElementById('summary-prompts-list');
  const addSummaryPromptBtn = document.getElementById('add-summary-prompt-btn');

  function renderSummaryPromptsList() {
    if (!summaryPromptsListEl) return;
    summaryPromptsListEl.innerHTML = '';
    summaryPromptsList.forEach(function(item, index) {
      const itemDiv = document.createElement('div');
      itemDiv.className = 'summary-prompt-item';

      const nameInput = document.createElement('input');
      nameInput.type = 'text';
      nameInput.className = 'summary-prompt-name-input';
      nameInput.value = item.name;
      nameInput.placeholder = '提示词名称';
      nameInput.addEventListener('change', function() {
        summaryPromptsList[index].name = this.value;
      });
      ...
      textarea.placeholder = '输入摘要提示词内容...';
      textarea.addEventListener('change', function() {
        summaryPromptsList[index].prompt = this.value;
      });

      const btnGroup = document.createElement('div');
      btnGroup.className = 'summary-prompt-btn-group';

      const saveBtn = document.createElement('button');
      saveBtn.className = 'save-prompt-btn';
      saveBtn.textContent = '保存';
      saveBtn.addEventListener('click', function() {
        saveSummaryPromptsList();
      });

      const deleteBtn = document.createElement('button');
      deleteBtn.className = 'delete-summary-prompt-btn';
      deleteBtn.textContent = '删除';
      if (summaryPromptsList.length <= 1) {
        deleteBtn.disabled = true;
        deleteBtn.style.opacity = '0.4';
        deleteBtn.style.cursor = 'not-allowed';
      }
      deleteBtn.addEventListener('click', function() {
        if (summaryPromptsList.length <= 1) return;
        summaryPromptsList.splice(index, 1);
        saveSummaryPromptsList();
        renderSummaryPromptsList();
      });
      ...
      summaryPromptsListEl.appendChild(itemDiv);
    });
  }

  function saveSummaryPromptsList() {
    chrome.storage.sync.set({ summary_prompts_list: summaryPromptsList }, function() {
  ...
  function loadSummaryPromptsListForSettings() {
    loadSummaryPromptsList(function(list) {
      summaryPromptsList = list;
      renderSummaryPromptsList();
    });
  }

  if (addSummaryPromptBtn) {
    addSummaryPromptBtn.addEventListener('click', function() {
      const newId = 'custom_' + Date.now();
      summaryPromptsList.push({
        id: newId,
        name: '自定义摘要 ' + summaryPromptsList.length,
        prompt: DEFAULT_PROMPTS.SUMMARY_PROMPT
      });
      renderSummaryPromptsList();
    });
  }

  loadSummaryPromptsListForSettings();
  // === 摘要提示词列表管理 end ===
